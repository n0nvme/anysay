...
from ricksay.prepare import prepare_file, add_files
...
if __name__ == "__main__":
    ...
    if debug: print(args_main)

    if args_main.add_files:
        if debug: print(args_main.add_files)
        ...
        if os.path.exists(pics_path) and len(os.listdir(pics_path)) > 0:
            ...
            if debug: print(os.path.normpath(os.path.join(pathlib.Path(__file__).parent.absolute(), "../picsgen"),))
            add_files(
                os.path.normpath(os.path.join(pathlib.Path(__file__).parent.absolute(), "../picsgen")),
                debug=debug
            )
